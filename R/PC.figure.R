...
#' PC.figure(PC)
#' PC %>% PC.figure()
#' spNitrates <- ChimieOrg %>% filter(pcmes_parametrenom == "Nitrates") %>% PC.figure()
#' spNitrates <- ChimieOrg %>% filter(pcmes_parametresandre == 1340) %>% PC.figure()
...
#### Contexte ####
  ...
  if(testit::has_error(PC %>% 
                       distinct(pcmes_parametresandre) %>% 
...
Figure <- Figure + ggtitle(Contexte$pcmes_parametrenom) + xlab("pcmes_date") + ylab(paste0("Concentration (", Contexte$pcmes_unitenom, ")"))
Figure
